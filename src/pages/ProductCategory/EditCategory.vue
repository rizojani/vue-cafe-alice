...
<template>
  <div class="content-body">
    ...
    <section id="configuration">
      ...
      <div class="clearfix"></div>
      <p class="clr_grey medium mt-1 p_sm">Edit Category</p>
      <div class="clearfix"></div>
...
export default {
  ...
  methods: {
    async getCategory(id) {
      try {
        let response = await this.axios.get(`api/product-category/edit/${id}`);
        this.name = response.data.cat.name;
        this.status = response.data.cat.status;
        let path =
          response.data.cat.url +
          "/storage/category/images/" +
          response.data.cat.image;
    ...
    async UpdateCategory() {
      try {
        ...
        fd.append("stock", this.status);
        fd.append("id", this.$route.params.id);
        let response = await this.axios.post(`api/product-category/update`, fd);
